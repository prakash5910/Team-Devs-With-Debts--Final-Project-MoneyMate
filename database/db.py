...
def update_user_info(user_id, name=None, username=None, email=None, password=None):
    conn = get_connection()
    c = conn.cursor()
    if name:     c.execute("UPDATE users SET name=?     WHERE id=?", (name,     user_id))
    if username: c.execute("UPDATE users SET username=? WHERE id=?", (username, user_id))
    if email:    c.execute("UPDATE users SET email=?    WHERE id=?", (email,    user_id))
    if password: c.execute("UPDATE users SET password=? WHERE id=?", (password, user_id))
    conn.commit()
    conn.close()
...
def update_user_settings(user_id, currency=None, theme=None, notifications=None):
    conn = get_connection()
    c = conn.cursor()
    if currency:           c.execute("UPDATE users SET currency=?      WHERE id=?", (currency,      user_id))
